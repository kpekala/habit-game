...
export class TasksService {
    ...
    fetchCompletedTasks() {
        ...
        return this.http.get<any>(
            environment.backendPath + 'api/task/completed',
            {
                params: params,
            }
        )
    ...
    addTask(body) {
        ...
        return this.http.post(
            environment.backendPath + 'api/task/add',
            taskBody
        )
    }
    ...
    fetchHabits() {
        const params = { email: this.authService.getEmail() }
        return this.http.get<HabitDto[]>(
            environment.backendPath + 'api/habit',
            {
                params: params,
            }
        )
    ...
    doHabit(habitId: number) {
        ...
        return this.http
            .post(environment.backendPath + 'api/habit/do', body)
            .pipe(
                tap(() => {
                    this.userService.fetchUserInformation().subscribe()
                })
    ...
    uploadPhoto(file: File, photoId: string) {
        const formData = new FormData()
        formData.append('file', file)
        formData.append('photoId', photoId)
        return this.http.post(
            environment.backendPath + 'api/task/upload-photo',
            formData
        )
    }
