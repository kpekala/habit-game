...
export class AuthService {
  ...
  public signup(email: string, userName: string, password: string) {
    ...
    return this.http.post(this.authPath + 'signup', body).pipe(
  ...
  public signin(email: string, password: string) {
    ...
    return this.http.post(this.authPath + 'signin', body).pipe(
      tap((response: AuthResponse) => {
        this.saveAuthDataLocally(response, email);
      })
    );
  ...
  isUserLoggedIn(): boolean {
    ...
    if (token == null || tokenExpirationDateString == null) return false;
